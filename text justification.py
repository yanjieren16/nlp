...
# Check whether width of text is given and obtain the width. If width is not given, report error.
maxwidth_char = data[0].rstrip()
if maxwidth_char.isdigit() == 0:
    raise ValueError('No required max width.')
else:
    line_length = int(maxwidth_char)


# Check whether text is multiple lists and get the indent
def islist(list_line):
    indent_len = 0
...
# Get the total number of sub-lists, decide the space of the indentation
sublist_count = 0
indent_count = []
for r in range(1, len(data)):
    indent_count.append(islist(data[r]))
    if data[r] != '\n':
        if data[r].strip()[0].isdigit() == 1:
            sublist_count += 1

if sublist_count == 0:
    indent_space = 0
else:
    if 0 < sublist_count < 10:
        indent_space = 3
    else:
        if 10 < count < 100:
            indent_space = 4
...
EXCEPTIONS = """
as-so-ciate as-so-ciates dec-li-na-tion oblig-a-tory phil-an-thropic reci-procity re-cog-ni-zance ref-or-ma-tion
...
class Solution:
    def textJustify(self, words, maxwidth):
        ...
        for i, word in enumerate(words):
            # If the there is still space in the current line, then put next word in current line
            if len(word) + curr_words_length + end - start <= maxwidth:
                end += 1
                curr_words_length += len(word)
                current_line = ' '.join(words[start:end])
            else:
                space = maxwidth - (curr_words_length + end - start + 1)
                last_word = hyphenate_word(word)
                sum = 0
                for j in range(len(last_word)):
                    sum += len(last_word[j])
                    if sum <= space:
                        first_half = ''.join(last_word[0:j])+'-'
                        last_half = ''.join(last_word[j:len(last_word)])
                if len(last_word[0]) + 1 + end - start + curr_words_length > maxwidth:
                    ...
                    if space > 0 and first_half != '-':
                        ...
                        curr_words_length = len(words[i])
        result.append(current_line)
        return result


# function to align plain text
def alignment(text):
    aligned_text = []
    total_space = 0
    for i in range(len(text)):
        current_line = str.split(text[i])
        word_num = len(current_line) - 1
        if word_num == 0 or i == len(text) - 1:
            aligned_text.append(' '.join(current_line))
        else:
            total_space = line_length - len(''.join(current_line))
            space, extra = divmod(total_space, word_num)
            for j in range(extra):
                if j % 2 == 0:
                    current_line[j//2] += ' '
                else:
                    current_line[len(current_line) - (j+1)//2 - 1] += ' '
            aligned_text.append((' ' * space).join(current_line))
    for j in range(len(aligned_text)):
            print(aligned_text[j])


# function to align multiple lists and sub-lists
def alignment_bulletin(list_leading, text, indentation):
    aligned_text = []
    total_space = 0
    for i in range(len(text)):
        current_line = str.split(text[i])
        ...
        if word_num == 0 or i == len(text) - 1:
            ...
            total_space = line_length - indentation - len(''.join(current_line))
            space, extra = divmod(total_space, word_num)
            for j in range(extra):
                if j % 2 == 0:
                    current_line[j//2] += ' '
    ...
    for j in range(len(aligned_text)):
        if j == 0:
            if '*' in list_leading:
                print(' ' * (indentation - 3) + list_leading + ' ' + aligned_text[j])
            else:
                print(' ' * (indentation - 2) + list_leading + ' ' + aligned_text[j])
        else:
            if '*' in list_leading:
                print(' ' * (indentation - 1) + aligned_text[j])
            else:
                print(' ' * (indentation + 1) + aligned_text[j])
...
line = ''
data.append('\n')
# set flag if previous line is a sub-list
flag = 0
...
def rmhyphen(words):
    for i in range(len(words)):
        if re.search(r'^[a-zA-Z]*-[a-z]*(ing)', words[i], flags=0):
...
for y in range(1, len(data)):
    if data[y] == '\n':
        if flag == 0:
            # get the words of current paragraph
            line_words = rmhyphen(line.split())
            # get the correct words in each line
            adjust_text = Solution().textJustify(line_words, line_length)
            # align the text to make if fully justified
            alignment(adjust_text)
            # print empty line after a paragraph
            print('')
            line = ''
        if flag == 1:
            print('')
    else:
        # check whether following line is a list and get the indentation before list
        indent = islist(data[y])
        if indent > 0:
            # remove the leading '1.' or '*'
            rm_leading_data = data[y].split()[1:len(data[y])]
            # get the text content without indentation
            bulletin = Solution().textJustify(rm_leading_data, line_length - indent * 4)
            # align the text with indentation
            alignment_bulletin(data[y].split()[0], bulletin, indent * 4)
            # set flag to indicate that the previous line is one of lists
            flag = 1
        else:
            if data[y][-2] == '-':
                # if last sentence ending with '-', connect it with the first word of next line
                line += data[y].rstrip() + ''
            else:
                line += data[y].rstrip() + ' '
            # to indicate that the previous line is not one of list
            flag = 0
